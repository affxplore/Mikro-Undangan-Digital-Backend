...
// Special endpoints for handling subscriptions with prices
...
// 1. RUTE PUBLIK (Tamu bisa kirim & lihat ucapan tanpa login)
ucapanRouter.post('/', upload.none(), createUcapanTamu); // Tamu kirim ucapan
ucapanRouter.get('/', getAllUcapanTamus);               // Tamu lihat daftar ucapan

// 2. RUTE PRIVATE (Hanya pemilik yang bisa edit/hapus, perlu login)
// Kita pasang verifyToken HANYA untuk rute di bawah ini
ucapanRouter.get('/:id', verifyToken, getUcapanTamuById);
ucapanRouter.put('/:id', verifyToken, upload.none(), updateUcapanTamu);
ucapanRouter.delete('/:id', verifyToken, deleteProject); // Eh, ini typo ya? Harusnya deleteUcapanTamu
...
// Rute pendaftaran (Bisa diakses user yang sudah login)
afiliasiRouter.post('/register', verifyToken, afiliasiController.createAfiliasi);

// Rute Admin
afiliasiRouter.get('/', verifyToken, hasRole('Super Admin', 'Owner','Admin'), afiliasiController.getAllAfiliasi);
afiliasiRouter.get('/:id', verifyToken, hasRole('Super Admin', 'Owner','Admin'), afiliasiController.getAfiliasiById);
afiliasiRouter.put('/:id', verifyToken, hasRole('Super Admin', 'Owner','Admin'), afiliasiController.updateAfiliasi);
afiliasiRouter.delete('/:id', verifyToken, hasRole('Super Admin', 'Owner','Admin'), afiliasiController.deleteAfiliasi);
