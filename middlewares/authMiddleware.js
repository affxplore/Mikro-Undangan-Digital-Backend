...
export const hasRole = (...allowedRoles) => {
    return (req, res, next) => {
        ...
        console.log(`[hasRole] Checking role: "${userRole}" (type: ${typeof userRole}) against allowed: [${allowedRoles.map(r => `"${r}"`).join(', ')}]`);
        
        if (userRole && allowedRoles.includes(userRole)) {
            return next(); // Lanjutkan jika role user termasuk dalam yang diizinkan
        }
        
        console.warn(`[hasRole] DITOLAK: role "${userRole}" tidak ada di [${allowedRoles.join(', ')}]`);
        return res.status(403).json({ 
            message: `Akses ditolak. Perlu role: ${allowedRoles.join(' atau ')}.`,
            yourRole: userRole,  // DEBUG: tampilkan role user di response
