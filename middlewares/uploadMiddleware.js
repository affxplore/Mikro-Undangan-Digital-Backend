...
// 2. Filter file khusus untuk ikon 
function categoryIconFilter(req, file, cb) {
  const ok = ["image/png", "image/svg+xml", "image/jpeg", "image/jpg"].includes(file.mimetype);
  if (!ok) return cb(new Error("Format file tidak didukung!"), false);
  cb(null, true);
}
