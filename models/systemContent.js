...
SystemContent.prototype.isActiveStatus = function() {
...
export async function DeleteDataForce(id, trx) {
  const item = await SystemContent.findByPk(id, {
    transaction: trx,
    lock: trx?.LOCK?.UPDATE,
  });
  if (!item) throw new Error("System content tidak ditemukan.");
  await item.destroy({ force: true, transaction: trx });
  return { message: "System content berhasil dihapus secara permanen." };
}
