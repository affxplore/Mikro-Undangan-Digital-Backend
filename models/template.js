...
const Template = db.define("Template", {
  ...
  schema: {
    type: DataTypes.TEXT,
    allowNull: true,
    comment: "Data JSON desain editor (warna, teks, background, decoration image dll)"
  },
...
export async function UpdateData(id, data, trx) {
  // 1. Validasi keberadaan data (opsional tapi bagus untuk error handling)
  const exists = await Template.findByPk(id, { transaction: trx });
  if (!exists) throw new Error("Template tidak ditemukan");

  // 2. Siapkan object update yang sangat bersih
  const updateFields = {
    title: data.title,
    label: data.label,
  ...
  // 3. Buang field yang undefined agar tidak menimpa data yang sudah ada
  Object.keys(updateFields).forEach(key => 
    updateFields[key] === undefined && delete updateFields[key]
  );

  console.log("🔥 [FINAL ATTEMPT] Direct Update Fields:", updateFields);

  // 4. GUNAKAN STATIC METHOD (Ini akan membypass Dirty Checking instance)
  await Template.update(updateFields, {
    where: { id: id },
    transaction: trx,
    logging: (sql) => console.log("🔍 [SQL EXECUTION]:", sql) // Kita intip SQL-nya di sini
  });

  // 5. Kembalikan data terbaru
  return await Template.findByPk(id, { transaction: trx });
}
...
export async function DeleteData(id, trx) {
  ...
  if (!template) {
    return {
      message: "Template sudah terhapus",
      deletedFiles: null
    };
  }
