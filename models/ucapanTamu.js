...
// ASSOCIATIONS
// ========================================
UcapanTamu.associate = function(models) {
  UcapanTamu.belongsTo(models.Invitation, {
    foreignKey: "invitation_id",
    as: "invitation",
  });
  
  UcapanTamu.belongsTo(models.ReceiveInv, { 
...
export async function DeleteData(trx, id) {
  ...
  //   offset,
  //   order: [["createdAt", "DESC"]],
  //   include: includeOptions, // <-- 2. Gunakan include di sini
  //   distinct: true, // <-- 3. Tambahkan distinct untuk akurasi paginasi
  // });
...
//   return await UcapanTamu.findByPk(id, {
//     include: [{ model: Invitation, as: "invitation", attributes: ["id", "name"] }]
//   });
// }

// Ubah CreateData agar bisa langsung include relasi
// export async function CreateData(data, trx) {
//   const newUcapan = await UcapanTamu.create(data, { transaction: trx });
//   // findByPk akan otomatis mengambil data terbaru dari DB
//   return await UcapanTamu.findByPk(newUcapan.id, {
//     transaction: trx,
//     include: [{ model: Invitation, as: "invitation", attributes: ["id", "name"] }]
//   });
// }
...
//   const item = await UcapanTamu.findByPk(id, {
//     transaction: trx,
//     lock: trx?.LOCK?.UPDATE,
//   });
//   if (!item) throw new Error("Ucapan tidak ditemukan.");
//   await item.update(data, { transaction: trx });
//   return item;
// }
...
//   if (!item) throw new Error("Ucapan tidak ditemukan.");
//   await item.destroy({ force: true, transaction: trx });
//   return { message: "Ucapan berhasil dihapus." };
// }
