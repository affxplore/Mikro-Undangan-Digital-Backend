...
// --- PERUBAHAN DI SINI: Ambil semua model dari index.js ---
import models from "../../models/index.js"; 
const { User, Role, AccessLv, Subscription, LoginAuth } = models;
// ---------------------------------------------------------

import { generateUniqueUsername } from "./userController.js";
import { sendVerificationEmail, sendResetPasswordEmail } from "../../helpers/emailService.js";

console.log("DEBUG: FRONTEND_URL dari env adalah:", process.env.FRONTEND_URL);
...
export async function login(req, res) {
  try {
    ...
    const accessToken = sign(payload, process.env.JWT_SECRET || 'mysecretkey', {
    expiresIn: "60m",
    });

    const refreshToken = sign(
    { id: user.id },
    process.env.JWT_SECRET || 'mysecretkey', // Gunakan JWT_SECRET sesuai .env kamu
    { expiresIn: "7d" }
    );
