...
export const getAllUcapanTamus = async (req, res) => {
  try {
    ...
    const includeOptions = [
      {
        model: Invitation,
        as: "invitation",
        attributes: ["id", "name"], // <-- DIUBAH DARI title MENJADI name
      },
    ];
    let parsedFilter = {};
    ...
    const data = result.data.map(ucapanTamuResponse);
...
export const createUcapanTamu = async (req, res) => {
  ...
  try {
    ...
    const newUcapan = await CreateData(payload, trx);
    await trx.commit();
...
export const updateUcapanTamu = async (req, res) => {
  ...
  try {
    ...
    const updatedUcapan = await UpdateData(id, payload, trx);
    await trx.commit();
