...
export const createFullInvitation = async (req, res) => {
  ...
  try {
    ...
    // Hilangkan slash di awal jika ada agar path.join tidak bingung
    const cleanPath = selectedTemplate.thumbnail_file.startsWith('/') 
        ? selectedTemplate.thumbnail_file.substring(1) 
        : selectedTemplate.thumbnail_file;

    const filePath = path.join(process.cwd(), 'public', cleanPath);
    console.log('[DEBUG] Mencari file di:', filePath);

    let fileContent;
    try {
      fileContent = await fs.readFile(filePath, 'utf-8');
    } catch (err) {
      console.error(`[ERROR] File tidak ditemukan di ${filePath}. Mencoba fallback...`);
      // Fallback: Coba baca langsung tanpa folder 'public' (jika path sudah lengkap)
